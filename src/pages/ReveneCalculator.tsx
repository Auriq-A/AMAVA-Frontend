...
export default function FBAProfitCalculatorPage() {
  ...
  const handleCalculate = async () => {
    ...
    try {
      const res=await fetch('http://localhost:51483/fba-profit-calculator',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({asin,cost:parseFloat(cost)})});
  ...
  const CalculateAllCheckedProduct = async () => {
    ...
    try {
      const res=await fetch('http://localhost:51483/revcalall',{method:'POST'});
